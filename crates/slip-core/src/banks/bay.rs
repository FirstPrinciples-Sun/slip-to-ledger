//! BAY (Krungsri / KMA / กรุงศรี) slip adapter.
...
#[derive(Default)]
pub struct BayAdapter;

impl BankAdapter for BayAdapter {
    ...
    fn detect(&self, ctx: &SlipContext) -> u8 {
        let t = &ctx.full_text;
        let mut score = 0u8;
        if t.contains("KMA") || t.contains("Krungsri") || t.contains("KRUNGSRI") {
            score = score.saturating_add(60);
        }
        if t.contains("กรุงศรี") || t.contains("BAY") {
    ...
    fn parse(&self, ctx: &SlipContext) -> crate::Result<PartialSlip> {
        ...
        let mut p = PartialSlip {
            source_bank: Some(SourceBank::Bay),
            adapter_version: Some("bay_v2026_05".into()),
        ...
        p.amount = parse_amount_near(
            t,
            &["จำนวนเงิน", "ยอดที่โอน", "ยอดเงิน", "Amount", "Transfer Amount"],
        );
...
fn parse_party(block: &str) -> Option<Party> {
    ...
    p.name = name_line;
    p.bank = Some("bay".into());
    Some(p)
}

register_adapter!(BayAdapter);
...
mod tests {
    ...
    #[test]
    fn detects_bay() {
        let c = ctx("KMA - Krungsri\nธนาคารกรุงศรีอยุธยา");
        assert!(BayAdapter.detect(&c) >= 90);
    }

    #[test]
    fn parses_bay_fixture() {
        let text = "\
KMA - Krungsri
โอนเงินสำเร็จ
15 พ.ค. 2569 14:32
...
บจก. เอบีซี จำกัด
xxx-x-x6789-0
ยอดที่โอน 1,500.00 บาท
ค่าธรรมเนียม 0.00 บาท
Reference No. BAY20260515ABC
";
        let c = ctx(text);
        assert!(BayAdapter.detect(&c) >= 60);
        let p = BayAdapter.parse(&c).unwrap();
        assert_eq!(p.amount, Some(1500.0));
        assert_eq!(p.source_bank, Some(SourceBank::Bay));
        assert_eq!(p.reference.as_deref(), Some("BAY20260515ABC"));
    }
}
