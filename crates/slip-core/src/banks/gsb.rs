...
#[derive(Default)]
pub struct GsbAdapter;

impl BankAdapter for GsbAdapter {
    ...
    fn detect(&self, ctx: &SlipContext) -> u8 {
        let t = &ctx.full_text;
        let mut score = 0u8;
        if t.contains("MyMo") || t.contains("MYMO") {
            score = score.saturating_add(60);
        }
        if t.contains("ออมสิน") || t.contains("GSB") {
    ...
    fn parse(&self, ctx: &SlipContext) -> crate::Result<PartialSlip> {
        ...
        let mut p = PartialSlip {
            source_bank: Some(SourceBank::Gsb),
            adapter_version: Some("gsb_v2026_05".into()),
            channel: Some(Channel::MobileApp),
            ..Default::default()
        };

        p.timestamp = parse_thai_datetime(t);
        p.amount = parse_amount_near(t, &["จำนวนเงิน", "ยอดเงิน", "ยอดที่โอน", "Amount"]);
        p.fee = parse_amount_near(t, &["ค่าธรรมเนียม", "Fee"]);
        p.reference =
            parse_reference_near(t, &["เลขที่อ้างอิง", "หมายเลขอ้างอิง", "รหัสอ้างอิง", "Reference"]);
...
fn parse_party(block: &str) -> Option<Party> {
    ...
    p.name = name_line;
    p.bank = Some("gsb".into());
    Some(p)
}

register_adapter!(GsbAdapter);
...
mod tests {
    ...
    #[test]
    fn detects_gsb() {
        let c = ctx("MyMo by GSB\nธนาคารออมสิน");
        assert!(GsbAdapter.detect(&c) >= 90);
    }

    #[test]
    fn parses_gsb_fixture() {
        let text = "\
MyMo by GSB
โอนเงินสำเร็จ
15 พ.ค. 2569 14:32
...
เลขที่อ้างอิง GSB20260515M
";
        let c = ctx(text);
        assert!(GsbAdapter.detect(&c) >= 60);
        let p = GsbAdapter.parse(&c).unwrap();
        assert_eq!(p.amount, Some(320.0));
        assert_eq!(p.source_bank, Some(SourceBank::Gsb));
        assert_eq!(p.reference.as_deref(), Some("GSB20260515M"));
    }
}
