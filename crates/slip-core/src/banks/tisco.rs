...
#[derive(Default)]
pub struct TiscoAdapter;

impl BankAdapter for TiscoAdapter {
    ...
    fn detect(&self, ctx: &SlipContext) -> u8 {
        let t = &ctx.full_text;
        let mut score = 0u8;
        if t.contains("TISCO") || t.contains("Tisco") {
            score = score.saturating_add(70);
        }
        if t.contains("ทิสโก้") {
    ...
    fn parse(&self, ctx: &SlipContext) -> crate::Result<PartialSlip> {
        ...
        let mut p = PartialSlip {
            source_bank: Some(SourceBank::Tisco),
            adapter_version: Some("tisco_v2026_05".into()),
...
fn parse_party(block: &str) -> Option<Party> {
    ...
    p.name = name_line;
    p.bank = Some("tisco".into());
    Some(p)
}

register_adapter!(TiscoAdapter);
...
mod tests {
    ...
    #[test]
    fn detects_tisco() {
        let c = ctx("TISCO Mobile\nธนาคารทิสโก้");
        assert!(TiscoAdapter.detect(&c) >= 90);
    }

    #[test]
    fn parses_tisco_fixture() {
        let text = "\
TISCO Mobile
โอนเงินสำเร็จ
15 พ.ค. 2569 14:32
...
";
        let c = ctx(text);
        assert!(TiscoAdapter.detect(&c) >= 60);
        let p = TiscoAdapter.parse(&c).unwrap();
        assert_eq!(p.amount, Some(600.0));
        assert_eq!(p.source_bank, Some(SourceBank::Tisco));
        assert_eq!(p.reference.as_deref(), Some("TIS20260515X"));
    }
}
