//! TTB (ttb touch / TMRW / ทหารไทยธนชาต) slip adapter.
...
#[derive(Default)]
pub struct TtbAdapter;

impl BankAdapter for TtbAdapter {
    ...
    fn detect(&self, ctx: &SlipContext) -> u8 {
        let t = &ctx.full_text;
        let mut score = 0u8;
        if t.contains("ttb touch") || t.contains("TMRW") {
            score = score.saturating_add(60);
        }
        if t.contains("ttb") || t.contains("TTB") {
            score = score.saturating_add(35);
        }
        if t.contains("ทหารไทยธนชาต") {
    ...
    fn parse(&self, ctx: &SlipContext) -> crate::Result<PartialSlip> {
        ...
        let mut p = PartialSlip {
            source_bank: Some(SourceBank::Ttb),
            adapter_version: Some("ttb_v2026_05".into()),
...
fn parse_party(block: &str) -> Option<Party> {
    ...
    p.name = name_line;
    p.bank = Some("ttb".into());
    Some(p)
}

register_adapter!(TtbAdapter);
...
mod tests {
    ...
    fn detects_ttb() {
        let c = ctx("ttb touch\nธนาคารทหารไทยธนชาต");
        assert!(TtbAdapter.detect(&c) >= 90);
    }

    #[test]
    fn parses_ttb_fixture() {
        let text = "\
ttb touch
โอนเงินสำเร็จ
15 พ.ค. 2569 14:32
...
เลขที่ทำรายการ TTB20260515Z
";
        let c = ctx(text);
        assert!(TtbAdapter.detect(&c) >= 60);
        let p = TtbAdapter.parse(&c).unwrap();
        assert_eq!(p.amount, Some(480.0));
        assert_eq!(p.source_bank, Some(SourceBank::Ttb));
        assert_eq!(p.reference.as_deref(), Some("TTB20260515Z"));
    }
}
